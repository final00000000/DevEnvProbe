...
import { deployService } from "../../src/services";
...
describe("DockerPage 部署流水线区块", () => {
  ...
  it("应渲染部署流水线卡片与主按钮", async () => {
    ...
    expect(content.textContent).toContain("仅拉取代码不会自动更新容器");
    expect(content.querySelector('[data-deploy-action="run-pipeline"]')).not.toBeNull();
  });
  ...
  it("Run 模式部署应先弹窗确认，取消后不执行", async () => {
    ...
    const runBtn = content.querySelector('[data-deploy-action="run-pipeline"]') as HTMLButtonElement;
    runBtn.click();
    await Promise.resolve();
